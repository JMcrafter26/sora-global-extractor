...
def get_functions(allowed_functions=[]):
    ...
    print(f"{Colors.CYAN}🔍 SCANNING EXTRACTOR FILES{Colors.END}")
    print(f"{Colors.CYAN}{'='*60}{Colors.END}")
    print(f"📁 Scanning directory: {Colors.BLUE}{extractors_dir}{Colors.END}")
    print(f"🔍 Found {Colors.YELLOW}{len(files)}{Colors.END} JavaScript files\n")    # from the files, check if they contain a line with "/* SCHEME START */" and a line with "/* SCHEME END */", if not skip the file
            ...
            if start != -1 and end != -1:
                ...
                if allowed_functions:
                    if function_name not in allowed_functions:
                        print(f"⚠️  {Colors.YELLOW}Function {function_name} not in allowed list - skipping{Colors.END}")
                        continue
                # add the function to the list
                extractor_functions[function_name] = scheme
                print(f"✅ {Colors.GREEN}Found extractor: {function_name}{Colors.END}")
            else:
                print(f"❌ {Colors.RED}Invalid scheme in {file} - missing markers{Colors.END}")
    
    print(f"\n{Colors.CYAN}📊 SCAN RESULTS{Colors.END}")
    print(f"{Colors.CYAN}{'='*60}{Colors.END}")
    print(f"✅ Valid extractors found: {Colors.GREEN}{len(extractor_functions)}{Colors.END}")
    print(f"{Colors.CYAN}{'='*60}{Colors.END}\n")
...
def build_extractors(functions):
    ...
    print(f"{Colors.CYAN}🔨 BUILDING EXTRACTORS{Colors.END}")
    print(f"{Colors.CYAN}{'='*60}{Colors.END}")
    print(f"📦 Processing {Colors.YELLOW}{len(functions)}{Colors.END} extractors...")
    ...
    with open(os.path.join(script_dir, 'output', 'extractors.js'), 'w', encoding='utf-8') as f:
        ...
        if duplicate_functions:
            print(f"⚠️  {Colors.YELLOW}Duplicate function names found: {duplicate_functions}{Colors.END}")
            print(f"🔧 {Colors.CYAN}Resolving by removing duplicates...{Colors.END}")
            for function in duplicate_functions:
                # remove the function from the list
                del functions[function]
        if duplicate_classes:
            print(f"⚠️  {Colors.YELLOW}Duplicate class names found: {duplicate_classes}{Colors.END}")
            print(f"🔧 {Colors.CYAN}Resolving by removing duplicates...{Colors.END}")
    ...
    print(f"✅ {Colors.GREEN}Extractors built successfully!{Colors.END}")
    print(f"📄 Generated: {Colors.BLUE}extractors.js{Colors.END}")
    print(f"📄 Generated: {Colors.BLUE}extractors.json{Colors.END}")
    print(f"{Colors.CYAN}{'='*60}{Colors.END}\n")

def build_global_extractor(functions, allowed_functions=[]):
    print(f"\n{Colors.CYAN}{'='*60}{Colors.END}")
    print(f"{Colors.CYAN}🌐 BUILDING GLOBAL EXTRACTOR{Colors.END}")
    print(f"{Colors.CYAN}{'='*60}{Colors.END}")
    print(f"🔧 Processing {Colors.YELLOW}{len(functions)}{Colors.END} extractors...")
    ...
    with open(global_extractor_file, 'r', encoding='utf-8') as f:
        ...
        print(f"📦 {Colors.CYAN}Integrating extractor functions...{Colors.END}")
        ...
        print(f"🔗 {Colors.CYAN}Generating provider cases...{Colors.END}")
        provider_cases = ''
        ...
        if not os.path.exists(os.path.join(script_dir, 'test', 'test_providers.txt')):
            print(f"⚠️  {Colors.YELLOW}test/test_providers.txt not found. Skipping test providers.{Colors.END}")
        else:
            print(f"🧪 {Colors.CYAN}Adding test providers...{Colors.END}")
        ...
        if allowed_functions:
            print(f"✅ {Colors.GREEN}Building production global extractor...{Colors.END}")
            ...
            with open(os.path.join(script_dir, '..', 'global_extractor.js'), 'w', encoding='utf-8') as f:
                f.write(content)
            print(f"📄 Generated: {Colors.BLUE}global_extractor.js{Colors.END}")
        else:
            print(f"⚠️  {Colors.YELLOW}Building development global extractor (all extractors)...{Colors.END}")
            ...
            with open(os.path.join(script_dir, 'output', 'global_extractor_all.js'), 'w', encoding='utf-8') as f:
                f.write(content)
            print(f"📄 Generated: {Colors.BLUE}global_extractor_all.js{Colors.END}")
        
    print(f"✅ {Colors.GREEN}Global extractor built successfully!{Colors.END}")
    print(f"{Colors.CYAN}{'='*60}{Colors.END}\n")
...
def build(allowed_functions=[]):
    ...
    if not os.path.exists(os.path.join(script_dir, 'output')):
        ...
        print(f"📁 {Colors.CYAN}Created output directory{Colors.END}")

    try:
        # build the extractors
        build_extractors(functions)
    except Exception as e:
        print(f"❌ {Colors.RED}Error building extractors: {str(e)}{Colors.END}")
        return
...
def test():
    print(f"\n{Colors.CYAN}{'='*60}{Colors.END}")
    print(f"{Colors.CYAN}🧪 TESTING GLOBAL EXTRACTOR{Colors.END}")
    print(f"{Colors.CYAN}{'='*60}{Colors.END}")
    print(f"🚀 {Colors.CYAN}Running test suite...{Colors.END}")
    ...
    print(f"⏳ {Colors.YELLOW}Waiting for test results... (timeout: {timeout}s){Colors.END}")
    
    while not os.path.exists(os.path.join(script_dir, 'test', 'test_results.json')):
        if time.time() - start_time > timeout:
            print(f"❌ {Colors.RED}Timeout waiting for test results.{Colors.END}")
            print(f"{Colors.CYAN}{'='*60}{Colors.END}\n")
            return
        time.sleep(1)
...
# }
    ...
    print(f"📊 {Colors.CYAN}Processing test results...{Colors.END}")
    ...
    with open(os.path.join(script_dir, 'test', 'test_results.json'), 'r', encoding='utf-8') as f:
        ...
        for provider, result in test_results.items():
            ...
            if result == 'passed':
                ...
                print(f"   ✅ {Colors.GREEN}{provider}: PASSED{Colors.END}")
                passed_count += 1
            else:
                new_table_content += '| ' + provider + ' | ❌ |\n'
    ...
    print(f"📝 {Colors.CYAN}Updating README.md...{Colors.END}")
    with open(os.path.join(script_dir, '..', 'README.md'), 'r', encoding='utf-8') as f:
        ...
        if start == -1 or end == -1:
            print(f"❌ {Colors.RED}Error: Table not found in README.md{Colors.END}")
            print(f"{Colors.CYAN}{'='*60}{Colors.END}\n")
            return
...
if __name__ == "__main__":
    print(f"\n{Colors.CYAN}{'='*60}{Colors.END}")
    print(f"{Colors.CYAN}🚀 SORA GLOBAL EXTRACTOR BUILD SYSTEM{Colors.END}")
    print(f"{Colors.CYAN}{'='*60}{Colors.END}")
    
    # build the extractors
    startTime = time.time()
    build()
    print(f"\n🎉 {Colors.GREEN}Build completed successfully!{Colors.END}")
    ...
    print(f"\n{Colors.CYAN}{'='*60}{Colors.END}")
    print(f"{Colors.CYAN}🎯 BUILDING PRODUCTION VERSION{Colors.END}")
    print(f"{Colors.CYAN}{'='*60}{Colors.END}")
    print(f"🔧 {Colors.CYAN}Building global extractor with {Colors.GREEN}{len(allowed_functions)}{Colors.CYAN} working extractors...{Colors.END}")
    build(allowed_functions=allowed_functions)

    endTime = time.time()
    print(f"\n{Colors.CYAN}{'='*60}{Colors.END}")
    print(f"{Colors.CYAN}✅ BUILD PROCESS COMPLETED{Colors.END}")
    print(f"{Colors.CYAN}{'='*60}{Colors.END}")
    print(f"⏱️  Total time taken: {Colors.GREEN}{str(round(endTime - startTime, 2))} seconds{Colors.END}")
    print(f"📦 Working extractors: {Colors.GREEN}{len(allowed_functions)}{Colors.END}")
